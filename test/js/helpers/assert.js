// Small chai.assert-compatible shim backed by node:assert, so the tests can
// keep their existing `assert.*` calls without depending on chai.
const nodeAssert = require('assert')

// Deep-equality that mirrors chai's deep-eql: enumerate keys with for..in so
// inherited enumerable accessors count too (the native SubsequenceMatch exposes
// word/matchIndices/score as enumerable prototype accessors, which node:assert's
// own-property-only deepEqual would miss).
...
function deepEql (a, b) {
  ...
  if (typeof a !== 'object' || typeof b !== 'object' || a === null || b === null) {
    return a === b || (a !== a && b !== b) // treat NaN as equal to NaN
  }
  ...
  for (const key of aKeys) {
    if (!bKeys.includes(key) || !deepEql(a[key], b[key])) return false
  }
  return true
}
...
assert.deepEqual = (actual, expected, message) => nodeAssert.ok(deepEql(actual, expected), message)
assert.notDeepEqual = (actual, expected, message) => nodeAssert.ok(!deepEql(actual, expected), message)
